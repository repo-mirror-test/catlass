...
## 使用ACOT开发Matmul算子
本示例主要展示如何基于ACOT快速搭建一个NPU上的BasicMatmul实现。示例中使用已提供的下层基础组件完成Device层和Kernel层组装，并调用算子输出结果。ACOT分层示意图见[api文档](api.md)。
### Kernel层算子定义
...
```
using DispatchPolicy = acot::matmul::MmadAtlasA2Pingpong<true>; //流水排布使用
using L1TileShape = acot::MatmulShape<128, 256, 256>; // L1基本块
using L0TileShape = acot::MatmulShape<128, 256, 64>; // L0基本块
using AType = acot::matmul::MatmulType<ElementA, LayoutA>;     //封装了A矩阵的数据类型和排布信息
using BType = acot::matmul::MatmulType<ElementB, LayoutB>;     //封装了B矩阵的数据类型和排布信息
using CType = acot::matmul::MatmulType<ElementC, LayoutC>;     //封装了C矩阵的数据类型和排布信息

using BlockMmad = acot::matmul::block::BlockMmad<DispatchPolicy,
    L1TileShape,
    L0TileShape,
    AType,
    BType,
...
```
using TileScheduler = typename acot::matmul::block::MatmulIdentityBlockSwizzle<>;
```
4. 基于上述组件即可完成BasicMatmul示例的Kernel层组装。
```
using MatmulKernel = acot::matmul::kernel::MatmulUniversal<BlockMmad, void, TileSchedule>;
```
### Device层算子定义
基于Kernel层组装的算子，完成核函数的编写。
1. 使用ACOT_GLOBAL修饰符定义Matmul函数，并传入算子的类型参数。
```
template <
    class LayoutA,
    class LayoutB,
...
```
2. MatmulUniversal的调用接口为`()`运算符，需要传入Params作为参数。
```
...
```
### 算子编译
使用cmake，调用`acot_example_add_executable`函数指定target名称和编译文件。如下所示，00_basic_matmul为target名称，basic_matmul.cpp为需要编译的文件。
```
# CMakeLists.txt
acot_example_add_executable(
    00_basic_matmul
    basic_matmul.cpp
)
```
...
```
# 编译examples内所有用例
bash scripts/build.sh acot_examples
# 编译指定用例
...
```
执行结果如下，表明基于ACOT编写的Kernel已经成功执行。
```
Compare success.
```
### 代码样例
